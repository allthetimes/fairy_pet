...
- 台词里的标点（句号/逗号/**省略号**）由豆包模型**自动自然停顿**——不用做任何事；
- 精确停顿（如 ……=停 1 秒）需要 SSML `<break>`，但**豆包语音合成 2.0 / 复刻 2.0 的音色
  官方明确「暂不支持 SSML」**（仅 1.0 音色支持）——2.0 路线下无法做精确停顿；
- 客户端分段合成（按 …… 切句、段间静音）评估过并放弃：属于代码 hack，
  且分段会破坏模型对整句韵律的衔接。等官方支持 SSML 后再实现。
