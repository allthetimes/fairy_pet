...
>
> 最后更新：2026-09-14（**读图能力核验：不可用 → 按用户指令暂停**）
> 当前里程碑：**M2 → M3 过渡段**（视觉复刻尚未收尾 → Electron 工程尚未开工）
>
...
- **未完成**：① 用**3 个新下载的 B 站视频**做二次对比校准；② 量化对比**当前模型 vs in-game 头像**的差异并修复；③ Electron 工程（M1+）；④ 角色卡；⑤ 一键发版脚本
...
需要两份：
- `concept/llm-character-card.md` — 角色设定（人称、语气、范例对话、自我认知）
- `concept/window-behavior.md` — 桌宠行为配置（待机状态、点击响应、随机台词池、对 LLM 接口契约）

参考已有材料：
- `concept/visual-spec.md` §1–4 描述了形态
- 用户尚未指定 Fairy 的「性格」——直接默认走 2.6+ 的温和慵懒调（HDD 设定里的甜美助手形象），不确定就先做最小版 + 标注 TODO

### 2.4 Electron 工程（M1 起） 🏗️ **未开工**

参考：`E:\mypro\tiny_alarm`（Electron 34 + Vite 6 + TS 5.7）。

**计划但不行动的清单**（等视觉闭环再开工）：
- M1：scaffold + main.ts + 透明窗 + 加载 `dist/fairy.html`
- M2：托盘 + 单实例锁 + 自启 + 快捷键（ESC 唤出 / 隐藏 / 退出）
- M3：鼠标穿透 + 穿透窗口 ↔ 正常窗口切换
- M4：鼠标躲避（边界回弹）
- M5：多显示器适配（拖到副屏继续浮动）
- M6：性能自适应（前台 60fps / 后台 15fps / 锁屏 0）
- M7：LLM 对话接口（OpenAI 兼容）
- M8：键盘输入条 + 选中小表情
- M9：打包 + `release.mjs` 一键发版
...
4. 写 LLM 角色卡（2.3）+ 截图给用户评审
5. 用户点头后开 Electron（M1 起）
6. git init 先补上（§3.11）→ 再 release.mjs
```
