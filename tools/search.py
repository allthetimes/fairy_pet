# -*- coding: utf-8 -*-
"""
tools.search — B 站检索（取代 bili_search.py）

子命令：
  bili [--keyword "..."]   检索 B 站「绝区零 Fairy」相关视频
"""
import argparse
import urllib.parse
...
def bili(keyword=None, queries=None):
    qs = [keyword] if keyword else (queries or DEFAULT_QUERIES)
    results = []
    ...
    for r in results:
        print(f"{r['dur']} | {r['href']} | {r['title'][:50]}")
    print('total:', len(results))


def main():
    p = argparse.ArgumentParser(description='fairy-pet 检索工具')
    p.add_argument('cmd', choices=['bili'])
    p.add_argument('--keyword', help='自定义搜索词（仅 bili）')
    args = p.parse_args()
    if args.cmd == 'bili':
        bili(keyword=args.keyword)
